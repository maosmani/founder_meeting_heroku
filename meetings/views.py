...
def sales(request):
	...
	return render(request,'meetings/sales.html')
...
#This view will show the attentee list in the admin dashboard!
@login_required
def admin_attendee_list(request):
	current_user = request.user
	print("The current user id is: ",current_user.id)
	print("The current admin key is: ",current_user.admin_key)
	context = {

	  'attendees': NewUser.objects.all().filter(admin_key = current_user.admin_key),

	}
	return render(request,'meetings/admin_attendee_list.html', context)
...
#@login_required
def user_attendee_list(request):
	current_user = request.user
	print("The current user id is: ",current_user.id)
	print("The current admin key is: ",current_user.admin_key)
	context = {
	...
	return render(request,'meetings/user_attendee_list.html', context)
@login_required
